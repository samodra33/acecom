...
//view role
$view_permission = DB::table('permissions')->where('name', 'grn-index')->first();
...
{!! Form::open(['route' => ['grn.destroy', $grn_id], 'method' => 'delete']) !!}
<div class='btn-group'>

@if($view_permission_active)

	<a href="{{ route('grn.show', $grn_id) }}" class='btn btn-warning btn-sm'>
...
@if($showButton)
	...
	@if($edit_permission_active)
		<a href="{{ route('grn.edit', $grn_id) }}" class='btn btn-primary btn-sm'>
